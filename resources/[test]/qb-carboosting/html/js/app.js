...
const PlayerContract = (id, index, price) => {
    if (crypto - price >= 0) {
...
const Contract = (id, index, price, vinPrice, player) => {
    ...
    if (player == true) {
        ...
        return (
			...
			<p class="accepted-contract-text">If you choose to VIN scratch it will cost an <br> additional ${vinPrice} Cash to claim ownership.</p>
			<div class="accepted-contract-optionButton-flex">
...
const ToggleQueue = () => {
    inQueue = !inQueue
    if (inQueue === true) { $.post('https://qb-carboosting/NUI:togglequeue', JSON.stringify({})); } else { $.post('https://qb-carboosting/NUI:togglequeue', JSON.stringify({})); }
    SetQueueButton()
}

const SetQueueButton = () => {
    if (inQueue === true) { document.getElementById("ToggleQueue").innerHTML = "Leave Queue"; } else { document.getElementById("ToggleQueue").innerHTML = "Join Queue"; }
}
    ...
    window.addEventListener("message", (event) => {
        ...
        contractData.forEach(element => {
            ...
            if (element.player) {
                return (
                    contractDiv.innerHTML +=
                    `                <div class="contract-cards margin-cards">
                    <div class="contract-cards" id="decline-id${element.id}">
                        ...
                        <div class="contract-details">
                            <ul class="contract-details-text">
                                ...
                                <p>Buy in: ${element.price} Cash</p>
                                <p id="timer${counter}">Expires In: </p>
                            </ul>
                        </div>
                        <div class="contract-buttons-flex">
                            <button id="start-contract${counter}" onclick={Contract(${element.id},${counter},${element.price},${vinPrices[0][element.tier]},${element.player})} class="contract-buttons">Start Contract</button>
