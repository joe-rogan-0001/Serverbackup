    ...
    while true do
        ...
        if isLoggedIn then
            ...
            if activeVehicles[plate] and GetPedInVehicleSeat(veh, -1) == ped then
                if activeVehicles[plate].curHack < activeVehicles[plate].maxHack then
                    -- Alert Cops
                    TriggerServerEvent('qb-carboosting:server:SendLocation', GetEntityCoords(veh), Framework.Shared.VehicleHashes[GetEntityModel(veh)].model, GetVehicleNumberPlateText(veh))
