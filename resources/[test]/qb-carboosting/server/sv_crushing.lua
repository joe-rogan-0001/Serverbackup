RegisterNetEvent('qb-carboosting:server:CrushVehicle', function()
    ...
    if Player.PlayerData.job.name == 'police' then
        ...
        MySQL.query('DELETE FROM player_vehicles WHERE vehicle = ? AND plate = ? and vinscratched = 1', {model, plate})

        -- Log
        if Shared.Debug then print("^3[qb-carboosting] ^5Deleted VIN Scratch Vehicle (Vehicle Crush) Model: " .. model .. " Plate: ".. plate .. "^7") end
        TriggerEvent("qb-log:server:CreateLog", "carboosting", "Vehicle Delete", "red", "**".. Player.PlayerData.name .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) Crushed VIN Vehicle, Model: " .. model .. " Plate: " .. plate)
    else
...
Framework.Functions.CreateCallback('qb-carboosting:server:CanCrushVehicle', function(source, cb)
    ...
    if veh == 0 or not Player.PlayerData.job.name == 'police' then cb(false) return end -- Player.PlayerData.job.type == 'leo'
    local model = Framework.Shared.VehicleHashes[GetEntityModel(veh)].model
    local plate = GetVehicleNumberPlateText(veh)
    local result = MySQL.scalar.await('SELECT vinscratched FROM player_vehicles WHERE vehicle = ? AND plate = ?', {model, plate})
