...
RegisterNetEvent('qb-carboosting:server:UpdateTracker', function(licenseplate, success)
    ...
    if success then
        ...
        if Shared.Debug then print("^3[qb-carboosting] ^5Updated (Success) " .. licenseplate .. " Current Hacks: " .. newAmount .. "^7") end
    else
...
RegisterNetEvent('qb-carboosting:server:SendLocation', function(coords, model, plate)
    local players = Framework.Functions.GetQBPlayers()
    for _, Player in pairs(players) do
        if (Player.PlayerData.citizenid == activeVehicles[plate].owner) or (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then -- Player.PlayerData.job.type == 'leo'
...
Framework.Functions.CreateCallback('qb-carboosting:server:GetPlayerVehicleLoc', function(source, cb, plate, target)
    local result = MySQL.scalar.await('SELECT garage FROM player_vehicles WHERE citizenid = ? AND plate = ?', {target, plate})
    cb(result)
end)
