...
RegisterNetEvent('qb-drugsystem:server:CornerSellToPed', function(netId, zone)
    ...
    for _, item in pairs(Player.PlayerData.items) do
        if Shared.CornerSellDrugs[item.name] then
            ...
            payout += math.random(Shared.CornerSellDrugs[item.name].baseMin, Shared.CornerSellDrugs[item.name].baseMax) * removeAmount -- Base Price
            ...
            if item.info.purity then -- Payout based on purity
                payout += Shared.CornerSellDrugs[item.name].purityMultiplier * item.info.purity * removeAmount
            end
