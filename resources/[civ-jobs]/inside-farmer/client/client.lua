...
AddEventHandler('QBCore:Client:OnJobUpdate', function()
	...
	elseif not Config.jobrequirement then
		...
		CreateFarmerBlip(false)
	end
end)

function CreateFarmerBlip(type)
	if type then
		RemoveBlip(blip)
		blip = AddBlipForCoord(JobStart.Pos.x, JobStart.Pos.y, JobStart.Pos.z)
...
Citizen.CreateThread(function()
	if not Config.jobrequirement then CreateFarmerBlip(true) end
	while true do
		local sleep = 500
		...
		if(Config.jobrequirement and (PlayerData.job ~= nil and PlayerData.job.name == Config.jobname)) or not Config.jobrequirement then
			if(GetDistanceBetweenCoords(coords, JobStart.Pos.x, JobStart.Pos.y, JobStart.Pos.z, true) < 8.0) and not JobStarted then
				...
				if(GetDistanceBetweenCoords(coords, JobStart.Pos.x, JobStart.Pos.y, JobStart.Pos.z, true) < 1.5) then
					DrawText3Ds(JobStart.Pos.x, JobStart.Pos.y, JobStart.Pos.z+1.4, 'To start the order, press [~g~E]')
				end
	...
	if(Config.jobrequirement and (PlayerData.job ~= nil and PlayerData.job.name == Config.jobname)) or not Config.jobrequirement then
		if(GetDistanceBetweenCoords(coords, JobStart.Pos.x, JobStart.Pos.y, JobStart.Pos.z, true) < 1.5) and not JobStarted then
			...
			if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) then
				...
				--  exports['mythic_notify']:DoCustomHudText('inform', '<b>Farmer</b></br>Job started, go to the warehouse!', 1500)
                QBCore.Functions.Notify("Farmer Job started, go to the warehouse!", 'primary')
				JobStarted = true
				WarehouseBlip()
			elseif IsControlJustReleased(0, Keys['E']) and IsPedInAnyVehicle(ped, false) then
				...
				-- exports['mythic_notify']:DoCustomHudText('inform', '<b>Farmer</b></br>Leave the vehicle!', 1500)
				QBCore.Functions.Notify("Leave the vehicle!", 'primary')
			end
		elseif(GetDistanceBetweenCoords(coords, Cloakroom.Pos.x, Cloakroom.Pos.y, Cloakroom.Pos.z, true) < 1.5) and JobStarted then
			...
			if IsControlJustReleased(0, Keys['E']) and not clothes and not IsPedInAnyVehicle(ped, false) then
				...
				-- exports['mythic_notify']:DoCustomHudText('inform', '<b>Farmer</b></br>Go to the container!', 1500)
				QBCore.Functions.Notify("Go to the container!", 'primary')
				ContainerBlip()
		...
		elseif(GetDistanceBetweenCoords(coords, Container.Pos.x, Container.Pos.y, Container.Pos.z, true) < 1.5) and clothes then
			...
			if IsControlJustReleased(0, Keys['E']) and not vegetable and not IsPedInAnyVehicle(ped, false) then
				...
				-- exports['mythic_notify']:DoCustomHudText('inform', '<b>Farmer</b></br>Today we will plant '..VegetableType.FarmName, 1500)
				QBCore.Functions.Notify("Today we will plant "..VegetableType.FarmName, 'primary')
...
function JobDone(type)
	...
	--exports['mythic_notify']:DoCustomHudText('inform', '<b>Farmer</b></br>The payout can be picked up', 1500)
	QBCore.Functions.Notify("The payout can be picked up", 'primary')
			...
			if((GetDistanceBetweenCoords(coords, Payout.Pos.x, Payout.Pos.y, Payout.Pos.z, true) < 3) and (AmountPayoutC > 0 or AmountPayoutP > 0 or AmountPayoutL > 0) and JobStarted) then
				...
				if(GetDistanceBetweenCoords(coords, Payout.Pos.x, Payout.Pos.y, Payout.Pos.z, true) < 1.5) and JobStarted then
					if IsControlJustReleased(0, Keys['E']) and JobStarted and not IsPedInAnyVehicle(ped, false) then
						...
						--  exports['mythic_notify']:DoCustomHudText('inform', '<b>Farmer</b></br>You have earned '..(Config.PayoutC*AmountPayoutC+Config.PayoutL*AmountPayoutL+Config.PayoutP*AmountPayoutP)..'$', 1500)
						QBCore.Functions.Notify("You have earned "..(Config.PayoutC*AmountPayoutC+Config.PayoutL*AmountPayoutL+Config.PayoutP*AmountPayoutP)..'$', 'primary')
