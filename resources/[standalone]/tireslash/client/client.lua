...
RegisterNetEvent('tireslash:slash', function()
            ...
            if closestTire ~= nil then
                if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
                    ...
                    if driverServId == 0 then
                        ...
                        TriggerServerEvent('tireslash:sync', driverServId, closestTire.tireIndex)
                    end
                    Wait((animDuration / 2) * 1000)
                    ClearPedTasks(PlayerPedId())
                    RemoveAnimDict(animDict)
                else
                    TriggerEvent('tireslash:notify', Language['already_slashed'])
                end
            end
        else
            TriggerEvent('tireslash:notify', Language['no_weapon'])
        end
    end
end)

RegisterNetEvent('tireslash:sync')
AddEventHandler('tireslash:sync', function(tireIndex)
	TriggerEvent('tireslash:notify', Language['car_slashed'])
