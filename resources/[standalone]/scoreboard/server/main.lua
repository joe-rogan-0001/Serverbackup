...
function AddPlayer(player, restart)
    ...
    if not restart then
        TriggerClientEvent('scoreboard:addPlayer', -1, Scoreboard.players[playerId])
        UpdateCounter()
    end
end
...
function PlayerLoaded(player)
    if player then
        AddPlayer(player)
        TriggerClientEvent('scoreboard:playerLoaded', GetSource(player), Scoreboard.players)
    end
end

function RemovePlayer(playerId)
    if playerId then
        TriggerClientEvent('scoreboard:removePlayer', -1, Scoreboard.players[playerId])
...
AddEventHandler('onResourceStart', function(name)
	if name == GetCurrentResourceName() then
		...
		if #players > 0 then
            ...
            UpdateCounter()
            TriggerClientEvent('scoreboard:playerLoaded', -1, Scoreboard.players)
		end
	end
end)