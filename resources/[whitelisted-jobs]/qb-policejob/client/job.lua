...
local function stash()
    CreateThread(function()
        while true do
            Wait(0)
            if inStash and PlayerJob.type == "police" then
...
local function trash()
    CreateThread(function()
        while true do
            Wait(0)
            if inTrash and PlayerJob.type == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "policetrash", {
                        maxweight = 4000000,
...
local function fingerprint()
    CreateThread(function()
        while true do
            Wait(0)
            if inFingerprint and PlayerJob.type == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
...
local function armoury()
    CreateThread(function()
        while true do
            Wait(0)
            if inArmoury and PlayerJob.type == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
...
local function heli(zone)
    CreateThread(function()
        while true do
            Wait(0)
            if inHelicopter and PlayerJob.type == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
...
local function impound()
    CreateThread(function()
        while true do
            Wait(0)
            if inImpound and PlayerJob.type == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
local function garage()
    CreateThread(function()
        while true do
            Wait(0)
            if inGarage and PlayerJob.type == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["helicopter"]) do
            local helioptions = {}
            if Heli then
                helioptions = { label = Lang:t("menu.remove_heli"), icon = 'fas fa-helicopter', jobType = "police",
                action = function()
                    TriggerEvent('qb-police:client:removeHelicopter')
                end,
            }
            else
                helioptions = { label = Lang:t("menu.spawn_heli"), icon = 'fas fa-helicopter', jobType = "police",
                    action = function()
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            inStash = true
            if PlayerJob.type == "police" and PlayerJob.onduty then
