...
local function IsVehicpolicewned(plate)
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == "police" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == "police" or Player.PlayerData.job.name == "judge" then
        local playerId = tonumber(args[1])
        local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
...
QBCore.Commands.Add('fine', Lang:t("commands.fine"), {{name = 'id', help = Lang:t('info.player_id')}, {name = 'amount', help = Lang:t('info.amount')}}, false, function(source, args)
    ...
    if biller.PlayerData.job.type == "police" then
        if billed ~= nil then
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if not OtherPlayer or (Player.PlayerData.job.name ~= "police" or Player.PlayerData.job.type ~= "police") or not Player.PlayerData.job.onduty then return end
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Player.Functions.GetItemByName("moneybag") or not item.info or item.info == "" or Player.PlayerData.job.type == "police" or not Player.Functions.RemoveItem("moneybag", 1, item.slot) then return end
...
QBCore.Functions.CreateUseableItem('filled_evidence_bag', function(source, item)
    ...
    if Player and Player.PlayerData.job.type == "police" then
...
QBCore.Functions.CreateCallback('police:GetDutyPlayers', function(_, cb)
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.type == "police" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(_, cb)
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.type == "police" and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName("handcuffs") and (Player.PlayerData.job.name ~= "police" or Player.PlayerData.job.type ~= "police")) then return end
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    if (Player.PlayerData.job.type == "police" or Player.PlayerData.job.name == "ambulance" or Player.PlayerData.job.type == "ems") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or Player.PlayerData.job.name ~= "police" or Player.PlayerData.job.type ~= "police" then return end
...
RegisterNetEvent('police:server:JailPlayer', function(playerId, time)
    ...
    if (not Player or not OtherPlayer) or not Player.PlayerData.job.type == "police" then return end
...
RegisterNetEvent('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    ...
    if IsVehicpolicewned(plate) then
