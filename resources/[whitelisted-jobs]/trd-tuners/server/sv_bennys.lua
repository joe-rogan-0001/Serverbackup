...
RegisterNetEvent('script-customs:server:attemptPurchase', function(type, upgradeLevel)
    ...
    exports['Renewed-Banking']:removeAccountMoney("tuner", repairCost)
        else
    ...
    elseif type == "performance" or type == "turbo" then
        if balance >= vehicleCustomisationPrices[type].prices[upgradeLevel] then
            TriggerClientEvent('script-customs:client:purchaseSuccessful', source)
            exports['Renewed-Banking']:removeAccountMoney("tuner", vehicleCustomisationPrices[type].prices[upgradeLevel])
        else
            TriggerClientEvent('script-customs:client:purchaseFailed', source)
        end
    else
        if balance >= vehicleCustomisationPrices[type].price then
            TriggerClientEvent('script-customs:client:purchaseSuccessful', source)
    exports['Renewed-Banking']:removeAccountMoney("tuner", vehicleCustomisationPrices[type].prices)
        else
            TriggerClientEvent('script-customs:client:purchaseFailed', source)
        end
    end
...
QBCore.Commands.Add('bennys', 'yes hmmm info idk', {}, false, function()
    export[trd-tuners-main]:EnterCustomsAdmin()
end, 'god')
