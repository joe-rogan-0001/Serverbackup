RegisterServerCallback('qs-smartphone:server:GetInvoices', function(source, cb)
            ...
            if invoices[1] ~= nil then
                for k, v in pairs(invoices) do
                    ...
                    if Ply ~= nil then
                        ...
                        local resultNumber = MySQL.Sync.fetchAll("SELECT charinfo FROM `"..userColumns.."` WHERE `identifier` = '"..v.sender.."'", {} )
                ...
                for k, v in pairs(invoices) do
                    if v.status == 'unpaid' then
                        ...
                        if Ply ~= nil then
                            ...
                            local resultNumber = MySQL.Sync.fetchAll('SELECT charinfo FROM '..userColumns..' WHERE identifier = @id', { ['@id'] = v.author_identifier } )
                            if resultNumber and resultNumber[1] then 
                                v.number = json.decode(resultNumber[1].charinfo).phone
                            end
                        end
            ...
            if invoices[1] ~= nil then
                for _, v in pairs(invoices) do
                    ...
                    if Ply ~= nil then
                        ...
                        local res = MySQL.Sync.fetchAll('SELECT charinfo FROM '..userColumns..' WHERE citizenid = ?', {v.sendercitizenid})
                        if res[1] ~= nil then
                ...
                for k, v in pairs(invoices) do
                    if v.status == 'unpaid' then
                        ...
                        if Ply ~= nil then
                            ...
                            local resultNumber = MySQL.Sync.fetchAll('SELECT charinfo FROM '..userColumns..' WHERE citizenid = @citizenid', { ['@citizenid'] = v.author_identifier } )
                            if resultNumber and resultNumber[1] then 
                                v.number = json.decode(resultNumber[1].charinfo).phone
                            end
                        end
...
RegisterServerCallback('qs-smartphone:server:CanPayInvoice', function(source, cb, amount, invoiceId)
    if Config.Framework == 'esx' then 
        local src = source
...
RegisterServerCallback('qs-smartphone:server:PayInvoice', function(source, cb, invoiceId)
    local Invoices = {}
    ...
    if Config.billingSystem == 'jim-payments' then
        ...
        if invoice and invoice[1] then
            ...
            if SenderPly then
                -- Connected Player
                Ply.Functions.RemoveMoney('bank', invoice[1].amount)
                SenderPly.Functions.AddMoney('bank', invoice[1].amount)
                MySQL.Async.execute('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
                TriggerClientEvent("qs-smartphone:sendMessage", SenderPly.PlayerData.source, Lang("INVOICE_PAYMENT_RECEIVED") .. invoice[1].amount, 'success')
                cb(true)
            else 
                -- Disconnected Player
                MySQL.Async.fetchAll("SELECT * FROM `"..userColumns.."` WHERE citizenid = @citizenid", { ['@citizenid'] = invoice[1].sendercitizenid }, function(result)
                    if result and result[1] then
                        Ply.Functions.RemoveMoney('bank', invoice[1].amount)
                        local decoded = json.decode(result[1].money)
                        decoded.bank = decoded.bank + invoice[1].amount
                        MySQL.Async.execute("UPDATE `"..userColumns.."` SET `money` = '"..json.encode(decoded).."' WHERE `citizenid` = '"..invoice[1].sendercitizenid.."'", {}) 
                        MySQL.Async.execute('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
                        cb(true)
                    end
                end)
