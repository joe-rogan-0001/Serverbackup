...
RegisterServerEvent('jim-payments:server:ATM:use', function(amount, billtype, baccount, account, society, gsociety)
	...
	elseif account == "savings" then
		local getSavingsAccount = MySQL.Sync.fetchAll('SELECT * FROM bank_accounts_new WHERE citizenid = ? AND account_type = ?', { Player.PlayerData.citizenid, 'Savings' })
		...
		if billtype == "withdraw" then
			if savbal >= amount then
				...
				MySQL.Async.execute('UPDATE bank_accounts_new SET amount = ? WHERE citizenid = ? AND record_id = ?', { savbal, Player.PlayerData.citizenid, getSavingsAccount[1].record_id }, function(success)
	...
	elseif account == "society" then
		if billtype == "withdraw" then
			...
			elseif tonumber(society) >= amount then
				...
				else exports['Renewed-Banking']:removeAccountMoney(tostring(Player.PlayerData.job.name), amount) end
			end
		elseif billtype == "deposit" then
			...
			elseif bankB >= amount then
				...
				else exports['Renewed-Banking']:addAccountMoney(tostring(Player.PlayerData.job.name), amount) end
				Player.Functions.RemoveMoney('bank', amount) Wait(1500)
...
QBCore.Functions.CreateCallback('jim-payments:ATM:Find', function(source, cb)
	...
	local result = MySQL.Sync.fetchAll('SELECT * FROM bank_accounts_new WHERE citizenid = ? AND account_type = ?', { Player.PlayerData.citizenid, 'Savings' })
    if result[1] then
        accountID = result[1].record_id
        savingBalance = result[1].amount
	else
		MySQL.Async.insert('INSERT INTO bank_accounts_new (citizenid, amount, account_type) VALUES (?, ?, ?)', { Player.PlayerData.citizenid, 0, 'Savings' }, function() completed = true end) repeat Wait(0) until completed == true
		local result = MySQL.Sync.fetchAll('SELECT * FROM bank_accounts_new WHERE citizenid = ? AND account_type = ?', { Player.PlayerData.citizenid, 'Savings' })
