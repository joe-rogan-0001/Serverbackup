...
/*     console.log(data) */
    deleteAllNodes()
    showApartment(data.apartments,data.showApartments)
    showHouses(data.houses,data.showHouse)
    showSpawn(true)
...
/*     console.log(bool)
 */    if(bool) {
        $(document).find('.setLogo').show()
        $(document).find('.logo_text').hide()
    }else {
        $(document).find('.setLogo').hide()
        $(document).find('.logo_text').show()
    }
}

// Show Houses
function showHouses(data,bool) {
    let insertHouse = $('#chooseHouses');
    insertHouse.html("")
    if(data) {
        console.log(data)
        Object.keys(data).map(function(value,i){
            let html = `
            <option value="${data[i].name}" data-type="${data[i].house}">${data[i].house}</option>
            `
            insertHouse.append(html)
        })
    }
    if(!bool) {
        $(document).find("#chooseHouse").hide()
    }
}

// Show Apartements
function showApartment(data,bool) {
...
/*         console.log(data) */
    ...
    if(!bool) {
        $(document).find("#chooseApartmentAccount").hide()
    }
}
...
// Trigger Spawn
$(document).on("click",".btn_enter_spawn", function() {
...
$(document).on("click", "#closeUi", function(){
    disableDev()
...
$(document).on('click', '.setupApps', function(evt){
    evt.preventDefault();
...
$(document).on('click', '#enterLastLocation', function(evt){
    $('#enterAnimation').addClass('closeAnimation');
    setTimeout(() => {
...
$(document).on("click", ".spawn_house", function(){
    $('#enterAnimation').addClass('closeAnimation');
    let apartment = $('#chooseHouses').val()
    let apartmentType = $('#chooseHouses').find(':selected').attr('data-type')
    setTimeout(() => {
        $.post('https://jerzys_spawn/enterHouse', JSON.stringify({apartment,apartmentType}))
    }, 3000)
    setTimeout(() => {
       $('#enterAnimation').removeClass('closeAnimation')
    }, 6000)
