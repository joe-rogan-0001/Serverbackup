...
RegisterNetEvent('jerzys_spawn:server:createNode', function (data)
    ...
    if isGod then
        local d = data;
        if d ~= nil then
            MySQL.query('INSERT INTO jerzys_spawns (cord_name,cord_description,x_cord,y_cord,z_cord,top_value,left_value,deleted,showall) VALUES (?,?,?,?,?,?,?,?,?)',{d.cordName,d.cordDesc,d.x_cord,d.y_cord,d.z_cord,d.top_value,d.left_value,false,1}, function (id)
